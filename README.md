...
- **개발 중 발생한 어려움 또는 한계점**
    - 프로필 이미지 변경 <br/>
          프로필 관리에서 바꿀 이미지 경로를 입력 하면 코든 내부에서 지정된 프로젝트 내부의 비트맵 이미지 경로로 복사를 하는 코드(copy 함수 구현)를 이용하여 이미지 변경.
       <br/>
    - 커서 내리기 및 백스페이스(커서 올리기) <br/>
          한글 2byte를 이용하여 한줄의 최대 적을 수 있는 글자 수와 최대 x좌표값, 최대 y좌표값을 이용하여 알고리즘을 이용하여 최대 글자수가 넘어가거나 좌표값을 넘어가면 커서가 내려가면서 작성이 되게끔 구현.  		     
          백스페이스는 공백을 이용하여 지워지는 것을 표현(한글은 2byte이므로 공백 두번으로 지워지는 것을 표현), 한줄의 최소 글자 수를 넘지 않는다면 커서가 자동으로 위로 올라가며 글자가 지워지는 것을 구현.
